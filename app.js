				...
				if(err != null){
					...
					sqlQuery("DELETE FROM test WHERE userID = ?", ["depreciated" + userID], (err, rows) => {
						...
						if(err != null){
							...
							sqlQuery("UPDATE herd SET userID = ? WHERE userID = ?", ["depreciated" + userID, userID], (err, rows) => {
								console.log("Deleting tests")
								...
								if(err != null){
									...
									sqlQuery("UPDATE cow SET userID = ? WHERE userID = ?", ["depreciated" + userID, userID], (err, rows) => {
										console.log("Deleting tests")
										...
										if(err != null){
											...
											sqlQuery("UPDATE test SET userID = ? WHERE userID = ?", ["depreciated" + userID, userID], (err, rows) => {
												console.log("Deleting tests")
						...
						if(err){
							...
							sqlQuery("SELECT * FROM user WHERE username = ? OR email = ? OR phone = ?", [username, email, phone], (err, objects) => {
								...
								if(objects[0] != undefined){
									...
									else if(phone.substring(2, 12) == '1111111111' || phone.substring(5, 12) == '1111111'){
										res.send("Phone Number is Invalid")
										return
									}
