...
app.get("/user-test-app", authorizeUser, (req, res) => {
	...
	else if(req.query.userID != null && req.query.herdID != null && req.query.startDate != null && req.query.endDate != null){

		console.log("\n\n\n\nSTART DATE AND END DATE\n\n\n\n\n");
		sqlQuery("SELECT * FROM test WHERE userID = ? AND herdID = ? AND date > ? AND date < ?", [encrypt(req.query.userID), encrypt(req.query.herdID), encrypt(req.query.startDate), encrypt(req.query.endDate)], (err, objects) => {
			if(err){
				return res.send("Error: " + err)
			}
			else{
...
app.post('/test', authorizeUser, (req, res) => { //NOT YET BEING VALIDATED
	...
	sqlQuery("SELECT * FROM cow WHERE userID = ? AND herdID = ? AND id = ?", [encrypt(req.body.userID), encrypt(req.body.herdID), encrypt(req.body.cowID)], (err, objects) => {
		if(err != null){
			...
			console.log("SUCCESS IN REQUEST 1")
			if(req.body.value != undefined){
				sqlQuery("INSERT INTO test (date, testType, units, value, cowID, userID, herdID, milkFever, followUpNum, testID, milivolts, daysInMilk, dryOffDay, mastitisHistory, methodOfDryOff, farmBreedingIndex, parity, reproductionStatus, numberOfTimesBred, lactationNumber, daysCarriedCalfIfPregnant, projectedDueDate, current305DayMilk, currentSomaticCellCount, linearScoreAtLastTest, dateOfLastClinicalMastitis, chainVisibleID, animalRegistrationNoNLID, damBreed) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [date, testType, units, value, cowID, userID, herdID, milkFever, followUpNum, testID, milivolts, objects[0].daysInMilk, objects[0].dryOffDay, objects[0].mastitisHistory, objects[0].methodOfDryOff, objects[0].farmBreedingIndex, objects[0].parity, objects[0].reproductionStatus, objects[0].numberOfTimesBred, objects[0].lactationNumber, objects[0].daysCarriedCalfIfPregnant, objects[0].projectedDueDate, objects[0].current305DayMilk, objects[0].currentSomaticCellCount, objects[0].linearScoreAtLastTest, objects[0].dateOfLastClinicalMastitis, objects[0].chainVisibleID, objects[0].animalRegistrationNoNLID, objects[0].damBreed], (err2, rows) => {
